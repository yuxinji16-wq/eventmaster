...
def create_budget_log(data: BudgetLogCreate, db: Session = Depends(get_db)):
    """创建预算日志"""
    return log_service.create(db, data.model_dump())
...
def get_budget_overview(year: str = Query(..., description="年份"), db: Session = Depends(get_db)):
    ...
    # 获取这些活动的预算
    budgets = db.query(Budget).filter(Budget.activity_id.in_(activity_ids)).all() if activity_ids else []

    # 统计
    total_planned = sum(b.total_amount for b in budgets)
    total_used = sum(b.used_amount for b in budgets)

    # 按类别统计
    budget_ids = [b.id for b in budgets]
    items = db.query(BudgetItem).filter(BudgetItem.budget_id.in_(budget_ids)).all() if budget_ids else []
    ...
    for item in items:
        ...
        category_stats[cat]["planned"] += item.planned_amount or 0
        category_stats[cat]["actual"] += item.actual_amount or 0
