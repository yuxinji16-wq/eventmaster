...
## 8. 现阶段差距与开发计划（2026-05-10）

### 8.1 P0（两周内）

- 补齐核心业务接口的后端鉴权与权限校验。
- 前端 Gemini 调用改为后端代理，避免前端持有敏感 key。
- 统一 `fileApi` 前后端契约（实现或下线）。
- 调整默认管理员初始化策略，避免固定默认口令依赖。

### 8.2 P1（30-60天）

- 引入 Alembic 迁移体系并建立基线。
- 扩展 E2E 到权限隔离和失败路径。
- 拆分前端 API 单文件为分域模块。

### 8.3 P2（60-90天）

- 生产数据库切换到 MySQL/PostgreSQL。
- 增加可观测与性能治理（指标、告警、慢查询、索引）。
